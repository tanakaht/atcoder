    ...
    if q[0] == 1:
        ...
        if piv is None:
            ...
            if a <= piv:
                ...
                np = bt.search_higher(piv, math.inf)
                left_sum += (np - piv) * (cnt // 2)
                right_sum -= (np - piv) * (cnt // 2)
                right_sum += a - np
                piv = np
        elif cnt % 2 == 1:
            if a < piv:
                np = bt.search_lower(piv, -1*math.inf)
                left_sum -= (piv - np) * (cnt // 2)
                right_sum += (piv - np) * (cnt // 2 + 1)
                left_sum += (np-a)
                piv = np
            else:
                right_sum += (a - piv)
