...
visit_p_ = set(visit_p)
kouho_ = set(kouho)
entire = set(sorted(range(1000), key=lambda x: max(dist[x][-1], dist[x+1000][-1]))[:200])
best_of_best_score, best_of_best_visit_p = math.inf, visit_p_
while time()-ts<=0.8:
    visit_p = set(random.sample(entire, 50))
    kouho = entire-visit_p
    ...
    for _ in range(1000):
        ...
        T = T*COOL
    if best_score < best_of_best_score:
        best_of_best_score, best_of_best_visit_p = best_score, set(best_visit_p)

visit_p = best_of_best_visit_p
kouho = set(sorted(range(1000), key=lambda x: max(dist[x][-1], dist[x+1000][-1]))[:200])
for i in visit_p:
    kouho.remove(i)
INITIAL_STATE = -5 # Initial position
T = 100 # Temperature
