...
def mapping_val(f, x):
    if f ==-1:
        return x
    return f
def composition_val(f, g):
    if f == -1:
        return g
    return f
id_val = -1
v = [0]*N
lst_val = LazySegTree(op_val, e_val, mapping_val, composition_val, id_val, v)
...
def mapping_cnt(f, x):
    if f ==-1:
        return x
    return f
def composition_cnt(f, g):
    if f == -1:
        return g
    return f
id_cnt = -1
v = [0]*N
lst_cnt = LazySegTree(op_cnt, e_cnt, mapping_cnt, composition_cnt, id_cnt, v)
vals = sorted(range(N), key=lambda x: S[x:])
print(vals)
