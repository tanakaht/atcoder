...
kaizyo_inv = [1]
tmp = 1
for i in range(1, max(K, R, G, B)+2):
    tmp = (tmp*i) % MOD
    kaizyo.append(tmp)
...
Rs = [0 if i<R_min else comb(R, i) for i in range(min(K, R)+1)]
Gs = [0 if i<G_min else comb(G, i) for i in range(min(K, G)+1)]
Bs = [0 if i<B_min else comb(B, i) for i in range(min(K, B)+1)]
ans = ntt.convolution(Rs, Gs)
ans = ntt.convolution(ans, Bs)
print(ans[K])
