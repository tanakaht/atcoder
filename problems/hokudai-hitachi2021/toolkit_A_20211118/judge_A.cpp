...
void Main::before(int time) {
  ...
  for (auto &g : A_Data.grids) {
    assert((size_t)time < g->predictedEnergyBalanceBase.size());
    ...
    if (v->type == 1) {
      for (size_t i = 0; i < v->plans.size(); i++) {
        ...
        if (p->type == PlanType::drop) {
          bool found = false;
          assert(v->numCarryingOrders == v->carryingOrders.size());
          for (int j = 0; j < v->numCarryingOrders; j++) {
            assert(v->carryingOrders[j] != nullptr);
    ...
    if (v->type == 1 && !v->updatestop) {
      ...
      if (!hasRecharge && !v->plans.empty()) {
        ...
        int maxNeed = std::min(static_cast<int>(static_cast<long long>(
                      ...
                      v->Delta_move;
        if (e < maxNeed) {
          int charge = v->V_max * static_cast<int>(A_Data.tMax * 0.01);
    ...
    if (v->type == 1) {
      ...
      for (auto &p : v->plans) {
        ...
        if (p->type == PlanType::recharge || p->type == PlanType::recharging) {
          ...
          int start = t;
          int e = p->charge;
          int vehicleMaxSpeed = v->V_max;
          assert(vehicleMaxSpeed != 0);
    ...
    if (!v->plans.empty()) {
      ...
      if (v->vertex1 != v->vertex2) {
        ...
        if (v->vertex1 != p->vertex) {
          ...
          if (p->type == PlanType::recharging) {
            int e = std::min(v->V_max, p->charge);
    ...
    for (auto &cmd : A_Data.solCommands) {
      if (cmd->vehicle->equals(v)) {
        ...
        if (time >= cmd->timeCharge && cmd->charge != 0 &&
          ...
          int e = std::min(std::abs(cmd->charge), v->V_max);
          if (cmd->charge < 0) {
            e = -e;
          }
  ...
  for (int step = 0; step < A_Data.numSteps; step++) {
    ...
    if (mode == 0 || mode == 8) {
      ...
      if (A_Data.numEnergyVehicles == 0) {
        ...
        assert(A_Data.numEnergyVehicles != 0);
      }
      ...
      if (std::abs(cmd1->charge) <= cmd1->vehicle->V_max) {
        continue;
      }
      ...
      if (std::abs(cmd2->charge) <= cmd2->vehicle->V_max) {
        continue;
      }
...
int Main::eval(int time, std::vector<Command *> &commands, int tLimit) {
  for (size_t i = 0; i < commands.size(); i++) {
    ...
    int end1 = start1 + (std::abs(cmd1->charge) + cmd1->vehicle->V_max - 1) /
                        cmd1->vehicle->V_max;
  ...
  for (int t = time; t < tLimit; t++) {
    for (auto &cmd : A_Data.evalCommands) {
      ...
      if (t >= cmd->timeCharge) {
        if (cmd->evalCharge > 0) {
          int e = std::min(cmd->evalCharge, cmd->vehicle->V_max);
    ...
    for (auto &g : A_Data.grids) {
      ...
      if (g->turnCharge > 0) {
        g->evalCharge +=
        std::min(g->turnCharge, std::min(g->C_g_max - g->evalCharge, g->V_Max));
      } else if (g->turnCharge < 0) {
        int min = std::min(g->evalCharge, g->V_Max);
        if (g->turnCharge < -min) {
...
bool judge::valid_charge(size_t id, int amount) {
  ...
  if (amount > vehicles[id].V_Max_EV) {
    ;
    return false;
  }
...
bool judge::valid_end_time(bool onAcc) {
  ...
  while (!info_day.empty()) {
    ...
    if (onAcc) {
      for (auto &slt : shlt.shelter_data) {
        if (slt.x == e.id) {
          assert((T_now - 1) / (T_max / N_div) < shlt.shelter_D.size());
  ...
  for (auto e : delta_grid_total) {
    ...
    int delta = e.second;
    int MinV2 =
    std::min(V_Max_Grid, g.nodes[vertex_id].max_cap - g.nodes[vertex_id].cap);
    ...
    int L_buy = 0;
    if (L > 0) {
      L_FE = std::min(L, Asset.FE_list[0].P_FE_max);
      L_buy = L - L_FE;
      if (L_buy <= 0) {
        L_buy = 0;
      }
...
bool judge::readInput1(FILE *fp) {
  ...
  V_Max_Grid = Asset.PCS_list[0].P_PCS_in;
  sts = true;
  return sts;
}
  ...
  for (i = 0; i < n; i++) {
    ...
    if (s.size() == 2) {
      ...
      int C_PV = grid_data.A_grid_PV * pv->C_PV_init +
                 grid_data.A_grid_PV * v[grid_data.x_grid_pos].l;
      ;
      this->C_total += C_PV;
    } else {
      break;
...
void judge::getScoreData(FILE *) {
  ...
  readLineSkipComment();
  curr_score.w_trans = std::stoi(s[0]);
  curr_score.w_ele = std::stoi(s[1]);
  curr_score.w_env = std::stoi(s[2]);
  curr_score.w_acc = std::stoi(s[3]);
  readLineSkipComment();
...
std::string judge::setScoreData(FILE *fp) {
  ...
  { if (fp != outlog_fp) { fprintf(fp, "%d %d %d %d\n", curr_score.w_trans, curr_score.w_ele, curr_score.w_env, curr_score.w_acc); } if (outlog_fp && fp != stderr) { fprintf(outlog_fp, "%d %d %d %d\n", curr_score.w_trans, curr_score.w_ele, curr_score.w_env, curr_score.w_acc); } }
                                                       ;
...
void judge::getGraphData(FILE *) {
  ...
  "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 904, N, M);
  v.resize(N);
  v_p.resize(N);
  for (size_t i = 0; i < N; i++) {
    readLineSkipComment();
...
std::string judge::setDemandData(FILE *fp, int day, int id) {
  ...
  if (day == -1) {
    ...
    { if (fp != outlog_fp) { fprintf(fp, "%d %d\n", demand_info.x, demand_info.sigma_D2); } if (outlog_fp && fp != stderr) { fprintf(outlog_fp, "%d %d\n", demand_info.x, demand_info.sigma_D2); } };
    size_t k;
    for (k = 0; k < N_div; k++) {
      if (k != 0) {
        { if (fp != outlog_fp) { fprintf(fp, " "); } if (outlog_fp && fp != stderr) { fprintf(outlog_fp, " "); } };
...
std::string judge::setShelterData(FILE *fp) {
  ...
  for (i = 0; i < shlt.N_shelter; i++) {
    { if (fp != outlog_fp) { fprintf(fp, "%d %d\n", shlt.shelter_data[i].x, shlt.shelter_data[i].p); } if (outlog_fp && fp != stderr) { fprintf(outlog_fp, "%d %d\n", shlt.shelter_data[i].x, shlt.shelter_data[i].p); } }
                                             ;
  }
  for (i = 0; i < N_div; i++) {
    if (i != 0) {
...
std::vector<double> judge::calcRadRValue(int day,
  ...
  assert((size_t)vertex_idx < v.size());
...
int judge::sample_vertex(const std::vector<int> &selection_table,
  ...
  while (right - left > 1) {
    ...
    assert((size_t)center < selection_table.size());
...
std::string judge::set_Algorithm_Initial_Input1(const bool submitFlag,
  ...
  if (false) fprintf(stderr, "###Debug %s(%d): before fprintf: N_sol=%zu\n",
                      "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 1652, N_sol);
  main.A_Data.numSol = N_sol;
  if (false) fprintf(stderr, "###Debug %s(%d): before fprintf: N=%zu,M=%zu\n",
                      "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 1656, N, M);
  ...
  if (main.A_Data.distances.size() == 0) {
    ...
    if (false) fprintf(stderr,
                        ...
                        Daytype, N_pattern);
    assert(M == (size_t)main.A_Data.numEdges);
    for (size_t i = 0; i < M; i++) {
      size_t v1 = u_output[i], v2 = v_output[i], d = c_output[i];
      --v1, --v2;
      assert((int)v1 < main.A_Data.numVertexes);
      assert((int)v2 < main.A_Data.numVertexes);
  ...
  if (false) {
    size_t grid_cnt = 0;
    for (auto const &g : main.A_Data.grids) {
  ...
  for (auto &g : main.A_Data.grids) {
    assert((size_t)g->vertex < main.A_Data.vertexToGrid.size());
  ...
  if (false) fprintf(stderr,
                      ...
                      "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 1935, N_nano, Grid_Info.N_grid);
  std::cerr << "pw:\n";
  ...
  for (size_t i = 0; i < (size_t)main.A_Data.numVehicles; i++) {
    assert(i < ev_info.data_list.size());
    ...
    main.A_Data.vehicles[i]->V_max = Asset.V_list[ev_data.type_EV].P_V_charge;
    ...
    if (false) {
      ...
      main.A_Data.vehicles[i]->C_e_max, main.A_Data.vehicles[i]->V_max,
  ...
  for (size_t i = 0; i < vehicles.size(); ++i) {
    ...
    vehicles[i].V_Max_EV = main.A_Data.vehicles[i]->V_max;
...
void judge::dumpTo2020InputFile(const Main &main) {
  char filename[128];
  {
    time_t rawtime;
    struct tm *timeinfo;
    time(&rawtime);
    timeinfo = localtime(&rawtime);
    strftime(filename, 128, "infile2020_%Y%m%d_%H%M%S.txt", timeinfo);
  }
  std::ofstream infile2020(filename);
  const char EOL = '\n';
  const char SPACE = ' ';
  ...
  for (auto *grid_data : A_Data.grids) {
    infile2020 << grid_data->vertex + 1
               << SPACE << grid_data->charge << SPACE << grid_data->C_g_init
               << SPACE << grid_data->C_g_max << SPACE << grid_data->V_Max
               << EOL;
  }
  infile2020 << "# N_vehicle C_Init_EV C_Max_EV V_Max_EV N_Max_Trans Delta_move"
             << EOL;
  ...
  for (int i = 0; i < A_Data.numVehicles; i++) {
    infile2020 << A_Data.vehicles[i]->vertex1 + 1
               << SPACE << A_Data.vehicles[i]->C_e_init << SPACE
               << A_Data.vehicles[i]->C_e_max << SPACE
               << A_Data.vehicles[i]->V_max << SPACE
               << A_Data.vehicles[i]->N_max << SPACE
               << A_Data.vehicles[i]->Delta_move << EOL;
  }
  infile2020 << "# p_trans_const Tlast" << EOL;
  infile2020 << 1.0 * Orders.size() / this->T_last << SPACE << this->T_last
             << EOL;
  infile2020 << "# p_trans gamma s_ele_ref s_trans_ref" << EOL;
  infile2020 << 100000 << SPACE << 2 << SPACE << 0 << SPACE << 0 << EOL;
  infile2020 << "# T_max" << EOL;
  infile2020 << A_Data.tMax << EOL;
  infile2020 << "# order list total size=" << this->T_last << EOL
             << "# Orders.size=" << Orders.size() << EOL
             << "# empty orders in list=" << this->T_last - Orders.size()
             << EOL;
  int start_idx = 0;
  for (const auto &o : Orders) {
    for (int i = start_idx; i < o.time; i++) {
      infile2020 << 0 << EOL;
    }
    infile2020 << 1 << EOL << o.id + 1
               << SPACE << o.w + 1 << SPACE << o.z + 1 << SPACE << o.time
               << EOL;
    start_idx = o.time + 1;
  }
  for (size_t i = start_idx; i < this->T_last; i++) {
    infile2020 << 0 << EOL;
  }
  infile2020 << "# nanogrid list rows=" << A_Data.tMax
             << " grids per row=" << A_Data.grids.size() << EOL;
  for (int i = 0; i < A_Data.tMax; i++) {
    for (size_t j = 0; j < A_Data.grids.size(); j++) {
      if (j != 0) {
        infile2020 << SPACE;
      }
      const Grid *grid = A_Data.grids[j];
      infile2020 << grid->vertex + 1
                 << SPACE << energies[i][j].actual;
    }
    infile2020 << EOL;
  }
}
...
void judge::dumpForDebuggingScore(std::ofstream &ev1,
  ...
  for (int i = 0; i < A_Data.numVehicles; i++) {
    ev1 << A_Data.vehicles[i]->vertex1 << SPACE << A_Data.vehicles[i]->vertex2
        ...
        << SPACE << A_Data.vehicles[i]->V_max << SPACE
...
int judge::calc_pw_error(const size_t day, const size_t id) {
  ...
  assert(day < demand.day_list.size());
  assert(id < demand.day_list[day].size());
...
int judge::createNanoGrid(const size_t day, Main &main) {
  ...
  for (size_t i = 0; i < T_max; i++) {
    for (size_t j = 0; j < N_nano; j++) {
      assert(j < pw.size());
      assert(i / (T_max / N_div) < pw[j].size());
      assert(i < energies.size());
      assert(j < energies[i].size());
...
std::string judge::run_Algorithm(const bool submitFlag,
  ...
  if (false) fprintf(stderr, "###Debug %s(%d),vehicles.size=%zu\n", "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp",
                      2553, vehicles.size());
  for (size_t i = 0; i < N_vehicle; i++) {
    if (false) fprintf(stderr, "###Debug %s(%d),i=%zu\n", "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 2557,
                        i);
    int u, v;
    size_t N_can_go, dist_u, dist_v;
    u = vehicles[i].now_vertex();
...
void judge::loadStat(StatusOnAcc &acc_stat) {
  for (size_t i = 0; i < Grid_Info.data_list.size(); i++) {
    ...
    assert(pos < acc_stat.grid_cap.size());
    Grid_Info.data_list[i].chg_grid_init = acc_stat.grid_cap[pos];
  }
  assert(acc_stat.vehicle_cap.size() == ev_info.data_list.size());
...
double run_alg(const bool submitFlag,
  ...
  if (onAcc) {
    for (auto &slt : Common.shlt.shelter_data) {
      assert(slt.x >= 0 && slt.x < main.A_Data.vertexToGrid.size());
...
double main2() {
  ...
  for (;;) {
    ...
    Common.readLineSkipComment();
    cmd = Common.StrLine;
    if (false) fprintf(stderr, "###Debug %s(%d),cmd=%s\n", "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 3115,
                        cmd.c_str());
    int argc = StringHelper::SplitToken(cmd, " ", cmd_args);
    if (false) fprintf(stderr, "###Debug %s(%d),argc=%d\n", "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 3118,
                        argc);
    ...
    if (cmd_args[0] == "test") {
      ...
      if (false) fprintf(stderr, "###Debug %s(%d),Common.N_day=%zu\n",
                          "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 3164, Common.N_day);
      ...
      ;
      total_score -= Common.curr_score.alpha_cost *
      ...
      "./tmp.upload.181704.guRngr/judge_A/judge_A.cpp", 3215, cmd_args[0].c_str());
      exit(1);
    }
    ;
    if (submitFlg) {
