    ...
    def joints_info(self, room: Room) -> List[Tuple[int, int, int, int, int]]:
        ...
        for i_id in dfs_ord[::-1]:
            if i_id in jointids:
                i = id2joint[i_id]
                i_id = root2id[uf.find(i)]
                ret.append((i//30, i%30, *_dfs(i_id)))
        return ret
...
#                     print(1111, x_, y_, s_close_pet, file=sys.stderr)
                    ret[x_][y_] = "#"
    ...
    def update_stage(self):
        ...
        if self.stage == 1:
            ...
            if self.satisfy(room):
                self.stage += 1
        if self.stage == 2:
            if self.satisfy(R2_(self), include_decimal=False):
                self.stage += 1
        if self.stage == 3:
            pass
        if self.stage == 4:
            if self.satisfy(R4_(self), include_decimal=False):
                self.stage += 1
        if self.stage == 5:
            pass
    ...
    def enum_turn(self, human: Human, dists: List[List[Optional[List[List[int]]]]], close_targets: Tuple[int, int], to_target: Optional[Tuple[int, int]]) -> Tuple[int, List[Operation]]:
        ...
        for i in range(len(close_targets)):
            ...
            best_d, best_d_, best_op = math.inf, math.inf, None
            for d in "udlr":
    ...
    def assign(self, ideal_R: Room) -> None:
                ...
                for g_id in groups.keys():
                    ...
                    for d in [1, -1]:
                        ...
                        if cur_dist[hum_id][op.x][op.y]+hum_turn[hum_id] < best_d:
                            best_d = cur_dist[hum_id][op.x][op.y]+hum_turn[hum_id]+1
                            best_pair = (hum_id, g_id, d, op, op_)
            hum_id, g_id, d, op, op_ = best_pair
            humans_close_target[hum_id] += groups[g_id][::d]
            is_done[g_id] = True
       ...
       # toをassign
        ...
        for human, ct, tt in zip(self.humans, humans_close_target, humans_to_target):
            task = self.enum_turn(human, dists, ct, tt)[1]
            human.task = Task(task)
            # print(f"{task}", file=logfile)

    def assign2(self, ideal_R: Room) -> None:
        for human in self.humans:
            human.task = Task([])
        m = len(self.humans)
        available = [[d!=math.inf for d in line] for line in ideal_R.dist(self.basepoint[0], self.basepoint[1])]
        ...
        is_assigned = [False]*len(self.humans)
        ...
        for i in range(len(self.humans)):
            if human_cmds[i] in "udlr":
                ...
                if not self.can_close(x+dx, y+dy):
                    human_cmds[i] = "."
        # 一旦UDLRで動く
        for human, cmd in zip(self.humans, human_cmds):
            if cmd not in "udlr":
                human.move(cmd)
